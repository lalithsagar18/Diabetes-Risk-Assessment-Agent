...
# Load model artifacts (cached to avoid reloading on every run)
@st.cache_resource
def load_model_artifacts():
    base_path = Path(__file__).parent.parent / "models"
    model = joblib.load(base_path / "best_model.pkl")
    scaler = joblib.load(base_path / "scaler.pkl")
    feature_order = joblib.load(base_path / "features.pkl")  # list of feature names in correct order
    label_map = joblib.load(base_path / "label_map.pkl")  # e.g., {0: "Low", 1: "High"}
    return model, scaler, feature_order, label_map
